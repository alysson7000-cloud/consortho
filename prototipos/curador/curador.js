...
function avaliarMemoria(memoria, elemento, estado) {
  const ciclosDesdeCriacao = (estado.ciclo || 0) - (memoria.ciclo || 0);
...
function curador() {
  ...
  const jardim = lerJSON(JARDIM_PATH, { elementos: {} });
  const sementes = lerJSON(SEMENTES_PATH, { sementes: [] });
  const construcoes = lerJSON(CONSTRUCOES_PATH, { construcoes: [] });
  const estado = lerJSON(ESTADO_PATH, { ciclo: 0 });
  ...
  for (const [elementoId, elemento] of Object.entries(jardim.elementos || {})) {
    ...
    // Atualizar elemento com apenas memórias vivas (mas manter histórico em visitas_gang)
    // NÃO removemos do jardim.json — apenas marcamos
  ...
  // Gerar relatório
  log(`📊 RELATÓRIO DE CURADORIA — Ciclo ${estado.ciclo}`);
  ...
  const sementesEmConstrucao = sementes.sementes?.filter(s => s.status === 'em_construcao') || [];
  const sementesProntas = sementes.sementes?.filter(s => s.status === 'prontas_para_construcao') || [];
  ...
  for (const s of sementesEmConstrucao) {
    const ciclosParada = (estado.ciclo || 0) - (s.ciclo_colheita || 0);
    if (ciclosParada > 500) {
      log(`⚠️ Semente "${s.origem}" parada há ${ciclosParada} ciclos — considerar promoção manual`);
    }
  }
